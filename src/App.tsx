  ...
  render() {
    return (
      <div className="row center ">
        ...
        <div
          className="col-12 toggle-container d-flex justify-content-start flex-row rounded-pill"
          id={`toggle-container-${this.state.id}`}
        >
          <div
            id={`toggle-switch-${this.state.id}`}
            onClick={this.toggle}
            className={`toggle-switch ${
              this.state.toggle === true ? "toggle-on" : "toggle-off"
            } rounded-pill`}
          />
        </div>
      </div>
...
class App extends Component<{}, AppState> {
  ...
  render() {
    return (
      <div className="container-fluid center m-2">
        <div
          id="drum-machine"
          className="row shadow rounded d-flex justify-content-around align-items-center p-4 main"
        >
          <div className="col-8">{this.renderDrumPads()}</div>
          <div className="col-4 row d-flex justify-content-around align-items-center">
            <Toggle
              className="col-12"
              label="Power"
              onChange={() => this.setState({ power: !this.state.power })}
            />

            <div className="col-12 text-center border mt-2" id="display">
              {this.state.lastKey}
            </div>
          </div>
        </div>
